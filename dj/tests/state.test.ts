...
import { mkdtempSync, mkdirSync, readFileSync, rmSync, statSync, writeFileSync, utimesSync, existsSync } from "node:fs";
...
let number = 0;
function setup() { const directory = join(root, String(number++)); return { directory, store: createStore(directory) }; }
const token = { access_token: "test-access", refresh_token: "test-refresh", expires_at: 1_800_000_000, scope: "user-read-playback-state" };
...
test("initialization imports placement and credentials together once, never the legacy palette", async () => {
  ...
  const legacyConfig = JSON.stringify({ spotify_client_id: "legacy-client", pi_placement: "above", palette: ["#000", "#000", "#000"] });
  writeFileSync(join(legacy, "config.json"), legacyConfig);
  writeFileSync(join(legacy, "spotify_tokens.json"), JSON.stringify(token));
  const directory = join(root, "imported"), store = createStore(directory, legacy);
  await store.initialize();
  expect(store.readSettings()).toEqual({ ...DEFAULTS, placement: "above" });
  expect(store.readCredentials()).toMatchObject({ clientId: "legacy-client", tokens: { ...token, expires_at: 1_800_000_000_000 } });
  await store.withLock(async lease => store.writeCredentials({ clientId: "new-client", tokens: { ...token, expires_at: 1_900_000_000_000 } }, lease));
  ...
  for (const name of ["settings.json", "spotify_tokens.json"]) expect(statSync(join(directory, name)).mode & 0o777).toBe(0o600);
});

test("fresh state defaults, scoped concurrent preferences, custom values and environment priority", async () => {
  const { directory, store } = setup(); await store.initialize();
  expect(store.readSettings()).toEqual(DEFAULTS);
  const second = createStore(directory);
  await Promise.all([store.updateSettings({ layout: "minimal" }), second.updateSettings({ placement: "above" })]);
  expect(store.readSettings()).toMatchObject({ layout: "minimal", placement: "above" });
  await store.updateSettings({ theme: { mode: "solid", colors: ["#abc"] }, customSolid: ["#abc"], customAnimated: ["#abc", "#fff"] });
  ...
  try {
    process.env.SPOTIFY_CLIENT_ID = "  env-client  "; expect(store.getClientId()).toBe("env-client");
    delete process.env.SPOTIFY_CLIENT_ID; expect(store.getClientId()).toBe("saved");
  } finally { if (old === undefined) delete process.env.SPOTIFY_CLIENT_ID; else process.env.SPOTIFY_CLIENT_ID = old; }
});

test("malformed settings, tokens, and cache are visible and never silently replaced", async () => {
  const { directory, store } = setup(); await store.initialize();
  for (const patch of [{ layout: ["full"] }, { placement: "left" }, { theme: { mode: "solid", colors: ["#fff", "#000"] } }, { theme: { mode: "animated", colors: ["#fff"] } }, { unknown: true }]) {
  ...
  for (const name of ["settings.json", "spotify_tokens.json", "playback.json"]) {
    const path = join(directory, name), previous = existsSync(path) ? readFileSync(path, "utf8") : undefined;
...
test("new state is not later overwritten by legacy files that appear after initialization", async () => {
  const directory = join(root, "fresh-before-legacy"), legacy = join(root, "later-legacy");
  const store = createStore(directory, legacy); await store.initialize();
  mkdirSync(legacy); writeFileSync(join(legacy, "config.json"), JSON.stringify({ spotify_client_id: "late", pi_placement: "above" }));
  writeFileSync(join(legacy, "spotify_tokens.json"), JSON.stringify(token));
  await store.initialize();
...
test("locks bound acquisition, recover stale owners, and prevent aborted or compromised writes", async () => {
  ...
  mkdirSync(lockPath); utimesSync(lockPath, new Date(0), new Date(0));
  await store.withLock(async lease => store.writeCredentials({ clientId: "recovered" }, lease));
  await store.withLock(async () => {
    await expect(store.withLock(async () => {}, undefined, false)).rejects.toMatchObject({ code: "ELOCKED" });
  });
  const aborted = new AbortController(); aborted.abort();
  await expect(store.withLock(async lease => store.writeCredentials({ clientId: "aborted" }, lease), aborted.signal)).rejects.toThrow();
  await expect(store.withLock(async lease => {
...
test("preference writes do not interfere with a pending Spotify lock", async () => {
  const { store } = setup(); await store.initialize();
...
test("cancellation interrupts lock contention before the other owner releases", async () => {
  ...
  await store.withLock(async () => {
    ...
    const waiting = store.withLock(async () => {}, controller.signal).then(() => "acquired", () => "aborted");
    controller.abort();
    expect(await Promise.race([waiting, new Promise(resolve => setTimeout(() => resolve("still waiting"), 500))])).toBe("aborted");
  });
});
...
test("preference locks also merge writes from separate processes", async () => {
  ...
  const children = [{ placement: "above" }, { layout: "medium" }].map(patch => Bun.spawn([process.execPath, "-e",
