...
test("hex input normalizes shorthand, rejects invalid tokens and enforces mode cardinality", () => {
  ...
  for (const input of ["", "   ", "red", "fff", "#ff", "#ffff", "#ggg", "#fff, #abc", "#fff garbage"]) {
...
test("three layouts preserve the original pastel cycle while only full includes elapsed time", () => {
  expect(plain(renderLine(playback, DEFAULTS, 80, 1000))).toBe(" ♪ M83 — Midnight City  1:24 / 3:42");
  expect(plain(renderLine(playback, { ...DEFAULTS, layout: "medium" }, 80, 1000))).toBe(" ♪ M83 — Midnight City");
  expect(plain(renderLine(playback, { ...DEFAULTS, layout: "minimal" }, 80, 1000))).toBe(" ♪ Midnight City");
  PASTELS.forEach((color, i) => {
    const line = renderLine(playback, DEFAULTS, 80, i * 90);
    expect(line).toBe(" " + colorize(plain(line).slice(1), color));
  });
  expect(renderLine(playback, { ...solid, layout: "minimal" }, 80, 0))
    .toBe(renderLine(playback, { ...solid, layout: "minimal" }, 80, 10000));
});
...
test("playing progress interpolates and clamps; paused and stale offline tracks stay frozen", () => {
  ...
  for (const status of ["paused", "offline"] as const) {
    ...
    expect(plain(renderLine(state, solid, 80, 999999))).toContain(`${status} · M83 — Midnight City  1:24`);
  }
  for (const status of ["idle", "offline", "paused", "auth"] as const) {
    expect(plain(renderLine({ status, sampledAt: 0 }, solid, 80, 0))).toContain(status === "auth" ? "/dj auth" : status);
  }
});

test("untrusted metadata is single-line, control-free and Unicode-safe at every terminal width", () => {
  const state: Playback = { ...playback, track: {
    ...playback.track!, name: "夜空 👩‍💻 é\n\t  Song\x1b[2J\x07\u202e",
  ...
  for (const layout of ["minimal", "medium", "full"] as const) {
    for (let width = 0; width < 100; width++) {
      expect(visibleWidth(renderLine(state, { ...solid, layout }, width, 1000))).toBeLessThanOrEqual(width);
    }
  }
