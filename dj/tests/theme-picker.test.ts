...
import type { ExtensionCommandContext, ExtensionUIContext, Theme as PiTheme } from "@earendil-works/pi-coding-agent";
import { CURSOR_MARKER, getKeybindings, visibleWidth, type Component, type Focusable, type TUI } from "@earendil-works/pi-tui";
import { pickLayout, pickTheme } from "../src/theme-picker.ts";
import { DEFAULTS, PASTELS, type Theme } from "../src/types.ts";

const down = "\x1b[B", up = "\x1b[A", enter = "\r", escape = "\x1b";
type Dialog = Component & Partial<Focusable> & { dispose?(): void };
type Step = (dialog: Dialog) => void;
const text = (dialog: Dialog) => stripVTControlCharacters(dialog.render(64).join("\n").replaceAll(CURSOR_MARKER, ""));
const keys = (...values: string[]): Step => dialog => values.forEach(value => dialog.handleInput!(value));
...
function ui(steps: Step[]) {
  ...
  const custom: ExtensionUIContext["custom"] = async (factory, options) => {
    ...
    const result = new Promise(resolve => { close = resolve; });
    const dialog = await factory(
      { requestRender() {} } as TUI,
      { fg: (_color: string, value: string) => value } as PiTheme,
      {} as Parameters<typeof factory>[2], close,
    ) as Dialog;
    try {
      dialog.focused = true;
    ...
    } finally { dialog.dispose?.(); disposed++; }
  };
  return {
    ctx: { ui: { custom } } as unknown as ExtensionCommandContext,
    verify() { expect(count).toBe(steps.length); expect(disposed).toBe(count); },
  };
}
...
test("final palette choice alone produces a settings patch, with real ANSI swatches", async () => {
  const fake = ui([keys(enter), dialog => {
...
test("back recreates disposed menus and root cancellation restores original preview", async () => {
  ...
  const previews: (Theme | undefined)[] = [];
  const fake = ui([
    dialog => { seen.add(dialog); keys(down, enter)(dialog); },
    dialog => { seen.add(dialog); keys(down, escape)(dialog); },
    dialog => {
...
test("custom solid prefills saved values, retains invalid input and saves normalized color", async () => {
  const fake = ui([keys(down, enter), keys(up, enter), dialog => {
  ...
  expect(await pickTheme(fake.ctx, { ...DEFAULTS, customSolid: ["#AABBCC"] }, () => {})).toEqual({
    theme: { mode: "solid", colors: ["#CC00FF"] }, customSolid: ["#CC00FF"],
  });
  fake.verify();
});
...
test("custom animated roundtrips, selects active custom theme, and cancels input back to palette", async () => {
  ...
  const settings = { ...DEFAULTS, theme: { mode: "animated" as const, colors }, customAnimated: colors };
  const fake = ui([keys(enter), dialog => {
  ...
  expect(await pickTheme(fake.ctx, settings, () => {})).toEqual({ theme: settings.theme, customAnimated: colors });
  fake.verify();
});

test("canceling a custom edit returns to Custom without changing saved settings", async () => {
  const settings = structuredClone(DEFAULTS);
  const fake = ui([keys(enter), keys(up, enter), keys("\x05", "\x15", "#bad", escape), dialog => {
    expect(text(dialog)).toContain("→ Custom…");
    keys(escape)(dialog);
  }, keys(escape)]);
...
test("native list and input honor customized keybindings", async () => {
  ...
  bindings.setUserBindings({ ...original,
    "tui.select.down": "ctrl+n", "tui.select.confirm": "ctrl+y", "tui.input.submit": "ctrl+t",
  });
  try {
    const fake = ui([keys("\x0e", "\x19"), keys(up, "\x19"), keys("\x05", "\x15", "#abc", "\x14")]);
    expect(await pickTheme(fake.ctx, DEFAULTS, () => {})).toEqual({
      theme: { mode: "solid", colors: ["#AABBCC"] }, customSolid: ["#AABBCC"],
    });
    fake.verify();
  } finally { bindings.setUserBindings(original); }
});

test("preview clears even when a dialog fails, and layout picker uses native selection", async () => {
  const previews: (Theme | undefined)[] = [];
  const fake = ui([() => { throw new Error("dialog closed"); }]);
  await expect(pickTheme(fake.ctx, DEFAULTS, theme => previews.push(theme))).rejects.toThrow("dialog closed");
  expect(previews.at(-1)).toBeUndefined();
  fake.verify();
  const ctx = { ui: { select: async (_title: string, options: string[]) => {
