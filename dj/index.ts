...
import { BorderedLoader, getAgentDir, type ExtensionAPI, type ExtensionCommandContext, type ExtensionContext } from "@earendil-works/pi-coding-agent";
...
const ARGUMENTS = [...LAYOUTS, "layout", "theme", "auth", "placement", "placement above", "placement below", "placement toggle"];
...
export default function dj(
  ...
  function paint() {
    ...
    const delay = animated ? 90 : playback.status === "playing" && settings.layout === "full" ? 1000 : 0;
    if (run && delay) paintTimer = setInterval(() => repaint?.(), delay);
    repaint?.();
  }

  function mount() {
    run?.ctx.ui.setWidget("dj", (tui) => {
      repaint = () => tui.requestRender();
      return {
        render: (width) => [renderLine(playback, { ...settings, theme: preview ?? settings.theme }, width)],
        invalidate() {},
      };
  ...
  async function login(ctx: ExtensionCommandContext) {
    let savedId = "";
    try { savedId = store.getClientId() ?? ""; }
    catch { ctx.ui.notify("Stored Spotify credentials are unreadable; successful authorization will replace them.", "warning"); }
    ...
    try {
      const result = await ctx.ui.custom<Error | undefined>((tui, theme, _keys, done) => {
        ...
        void spotify.authorize(clientId.trim(), async (url) => {
        ...
        }, signal).then(() => done(undefined), (error) => done(error instanceof Error ? error : new Error(String(error))));
        return loader;
      });
      if (auth !== current) return;
      if (!current.signal.aborted) ctx.ui.notify(result ? `DJ: ${result.message}` : "Spotify connected", result ? "error" : "info");
    } finally {
      if (auth === current) {
        auth = undefined;
        await start(ctx);
  ...
  pi.on("session_start", (_event, ctx) => { disposed = false; return start(ctx); });
  ...
  pi.registerCommand("dj", {
    ...
    getArgumentCompletions: (prefix) => ARGUMENTS.filter((value) => value.startsWith(prefix.toLowerCase())).map((value) => ({ value, label: value })),
    handler: async (args, ctx) => {
      ...
      try {
        if (!value) {
          ...
          const placement = requested === "above" || requested === "below" ? requested : settings.placement === "above" ? "below" : "above";
          await save({ placement }, ctx);
        } else if (value === "theme") {
          const selected = await pickTheme(ctx, settings, (theme) => { preview = theme; paint(); });
          if (selected) await save(selected, ctx);
        } else if (value === "auth") {
          await login(ctx);
        } else {
          ctx.ui.notify("Usage: /dj [minimal|medium|full|layout|theme|auth|placement [above|below|toggle]]", "info");
        }
      } catch (error) {
